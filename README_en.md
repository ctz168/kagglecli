...
Kaggle does not support one-click badges like Colab, so import the server notebook manually:

1. Download [`kaggle_server.ipynb`](kaggle_server.ipynb) from this repo
2. On Kaggle: **Create → New Notebook → File → Import Notebook** and upload it
3. In notebook **Settings**: turn **Internet → ON** (phone-verified account required); optionally pick a **GPU accelerator** (T4 x2 / P100)
4. **Run All** — Cell 3 prints the public URL, e.g. `https://xxxx.aitun.cc/xxxx`
